...
Base: ${q.breakdown.baseAED.toFixed(2)}
Fuel: ${q.breakdown.fuelAED.toFixed(2)}
Remote: ${q.breakdown.remoteAED.toFixed(2)}
Insurance: ${q.breakdown.insuranceAED.toFixed(2)}
  ...
  function emailRow(q: Quote) {
    ...
    const body = encodeURIComponent(
      `Please confirm/advise best rate for the following:\n\n` +
        ...
        `Carrier: ${q.carrier}\n` +
        `ETA: ${q.etaDays} days\n` +
        `Chargeable Weight: ${q.chargeableKg.toFixed(2)} kg\n` +
        `Total (AED): ${q.priceAED.toFixed(2)}\n\n` +
        `(Non-binding estimate generated by the calculator.)`
    );

    window.location.href = `mailto:support@crossbordercart.com?subject=${subject}&body=${body}`;
  }
    ...
    <Modal show={show} onHide={resetAndClose} centered size="lg">
      ...
      <Modal.Body>
        ...
        <Alert variant="light" className="mt-3 mb-0">
          These are <strong>non-binding estimates</strong>. Final rates are
          confirmed after review of package details, carrier rules, remote-area
          surcharges, and availability.
        </Alert>
        ...
        {!loading && quotes.length > 0 && (
          <>
            <Table hover responsive className="mt-3 mb-0">
              ...
              <tbody>
                {quotes.map((q, i) => (
                  <tr key={i}>
                    <td>{q.carrier}</td>
                    <td className="text-capitalize">{q.speed}</td>
                    ...
                    <td className="text-end">
                      <div className="d-flex gap-2 justify-content-end">
                        ...
                        <Button size="sm" onClick={() => emailRow(q)}>
                          Email me
                        </Button>
                      </div>
                    </td>
            ...
            <div className="small text-muted mt-2">
              Estimates only. Final charges may vary after carrier audit or
              remote-area validation.
            </div>
          </>
        )}
      </Modal.Body>
